...
Router.get('/maxproprofilecodesf330id', function (req, res) {
    ...
    Router.get('/duplicateitemid/:profilecodesf330id/:projectid', function (req, res) {
        // console.log("from maxempid"); 
        ...
        // mysqlConnection.query("SELECT * FROM emp_main WHERE emp_main.EmployeeID=?", req.param('empid'), (err, rows) => {
            ...
            var filter = [req.param('profilecodesf330id'), req.param('projectid')];
            // mysqlConnection.query("SELECT COUNT(*) AS employeeidcount FROM pro_team  WHERE pro_team.EmpID=?", req.param('empid'), (err, rows) => {
            mysqlConnection.query("SELECT COUNT(*) AS itemidcount FROM pro_profilecodesf330  WHERE pro_profilecodesf330.ProfileCodeSF330=? and pro_profilecodesf330.ProjectID=? " , filter, (err, rows, fields) => {
...
Router.get('/proprofilecodesf330details/:id', async (req, res) => {
    ...
    // Router.post('/', function (req, res) {
        ...
        await utils.maxid("pro_profilecodesf330","ID")
        .then(data => {
            maxid = data;
        });
...
// Router.post('/empdegree-angular-datatable/:empid', function (req, res) {
    Router.post('/proprofilecode-angular-datatable', async function (req, res) { // sending empid in body now
        ...
        await utils.totaldata("SELECT COUNT(ID) AS total FROM pro_profilecodesf330 WHERE pro_profilecodesf330.ProjectID="+ req.body.projectid)
        .then(data => {
            totalData = data;
        });
