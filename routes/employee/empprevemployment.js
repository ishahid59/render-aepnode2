...
Router.get('/maxempprevemploymentid', function (req, res) {
    // console.log("from maxempid"); 
    mysqlConnection.query("SELECT MAX(ID) as maxempprevemploymentid FROM  emp_prevemployment", (err, result) => {
        if (err) {
            console.log(err)
        }
        res.send(result);
...
Router.get('/:id',  (req, res) => {
    ...
    let sql = "SELECT * FROM emp_prevemployment WHERE emp_prevemployment.ID=?";
...
Router.post('/update', [
    ...
    check('company', "Company cannot be empty.").isInt({ min:1}),
    ...
    function (req, res) {
        ...
        let query = `UPDATE emp_prevemployment  SET ? WHERE ID=?`;
...
// Router.post('/', function (req, res) {
    ...
    Router.post('/',[ 
        ...
        await utils.maxid("emp_prevemployment","ID")
        .then(data => {
            maxid = data;
        });
...
// return;
    ...
    mysqlConnection.query('INSERT INTO emp_prevemployment SET ?', postdata, function (error, results, fields) {
        if (!error) {
            console.log("success");
            res.send(results);
        } else {
...
// DELETE pro team
Router.delete('/:empprevemploymentid', function (req, res) {
    mysqlConnection.query("DELETE FROM emp_prevemployment WHERE ID=?", req.param('empprevemploymentid'), (err, rows, fields) => {
        if (!err) {
            res.send(rows);
        } else {
            console.log(err);
...
Router.get('/empprevemploymentdetails/:id', async (req, res) => {
    ...
    mysqlConnection.query(
        ...
        `SELECT emp_prevemployment.ID, emp_prevemployment.EmpID, com_main.CompanyName AS disCompanyName,emp_prevemployment.EmpStartDate,emp_prevemployment.EmpStartYear,emp_prevemployment.EmpEndDate,emp_prevemployment.EmpEndYear,emp_prevemployment.JobTitle,emp_prevemployment.Notes  
        FROM emp_prevemployment LEFT OUTER JOIN 
        com_main ON emp_prevemployment.Company = com_main.ComID
        WHERE (emp_prevemployment.ID = ?)`
    ,
...
// Router.post('/empdegree-angular-datatable/:empid', function (req, res) {
    Router.post('/empprevemployment-angular-datatable', async function (req, res) { // sending empid in body now
        ...
        await utils.totaldata("SELECT COUNT(ID) AS total FROM emp_prevemployment WHERE emp_prevemployment.EmpID="+ req.body.empid)
        ...
        FROM emp_prevemployment LEFT OUTER JOIN 
        com_main ON emp_prevemployment.Company = com_main.ComID
        WHERE (emp_prevemployment.EmpID= `+ req.body.empid + `)`
        ...
        if (search == "") {
            ...
            // console.log(sql);
            sql = sql + ` AND com_main.CompanyName LIKE '%${search}%'`;
