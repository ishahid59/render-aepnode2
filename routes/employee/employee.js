...
Router.get('/duplicateemployeeid/:empid', function (req, res) {
    ...
    // mysqlConnection.query("SELECT * FROM emp_main WHERE emp_main.EmployeeID=?", req.param('empid'), (err, rows) => {
        ...
        mysqlConnection.query("SELECT COUNT(*) AS employeeidcount FROM emp_main  WHERE emp_main.EmployeeID=?", req.param('empid'), (err, rows) => {
...
// 2024 USING THIS (with count in same query)
