...
Router.get('/comdetails/:comid', function (req, res) {
    // mysqlConnection.query("SELECT * FROM emp_main WHERE emp_main.empid="+req.param('empid'),(err,rows,fields)=>{
    mysqlConnection.query(

    `SELECT DISTINCT com_main.ComID, com_main.CompanyName, com_main.AddressLine1, com_main.AddressLine2, com_main.City, list_state.str1 AS State, com_main.Zipcode, list_country.Str1 AS Country, 
    list_comocategory.Str1 AS OfficeCategory, list_comtownership.Str1 AS TypeOfOwnership, com_main.YearEstablished, com_main.TotalPersonnel, com_main.ParentCompanyName, com_main.DunsNo, 
    com_main.ParentCompany, com_main.IECompany  
    FROM com_main LEFT JOIN 
    list_state ON com_main.State =  list_state.ListID LEFT JOIN 
    list_country ON com_main.Country = list_country.ListID LEFT JOIN 
    list_comocategory ON com_main.OfficeCategory =  list_comocategory.ListID LEFT JOIN 
    list_comtownership ON com_main.TypeOfOwnership =  list_comtownership.ListID 
    WHERE (com_main.ComID = ?)`
    ,

     req.param('comid'), (err, rows, fields) => {
        if (!err) {
            res.send(rows[0]);
...
Router.get('/:comid', function (req, res) {
    ...
    // mysqlConnection.query("SELECT * FROM emp_main WHERE emp_main.EmpID=?", req.param('empid'), (err, rows, fields) => {
         mysqlConnection.query("SELECT * FROM com_main WHERE com_main.ComID=?", req.param('comid'), (err, rows, fields) => {
...
// Router.post('/', function (req, res) {
    ...
    Router.post('/',[ 
       ...
       async function (req, res) {
        ...
        await utils.maxid("com_main","ComID")
        .then(data => {
            maxid = data;
        });
    ...
    mysqlConnection.query('INSERT INTO com_main SET ?', postdata, function (error, results, fields) {
        if (!error) {
            console.log("success");
            res.send(results);
        } else {
...
Router.post('/update/', [
    ...
    function (req, res) {
        ...
        let query = `UPDATE com_main  SET ? WHERE ComID=?`;
        mysqlConnection.query(query, [post3, req.body.comid], (err, rows, fields) => {
            if (!err) {
                res.send(rows);
                console.log(post3);
            } else {
...
Router.delete('/:comid', function (req, res) {
    ...
    mysqlConnection.query("DELETE FROM com_main WHERE ComID=?", req.param('comid'), (err, rows, fields) => {
...
Router.post('/angular-datatable', async function (req, res) {
    ...
    // const value = await totaldata();
        ...
        `SELECT DISTINCT com_main.ComID, com_main.CompanyName, list_state.str1 AS State, list_country.Str1 AS Country, com_main.IECompany,
        (select count(*) from com_main WHERE com_main.ComID>0) as totaldata,
        (select count(DISTINCT com_main.ComID) ${from} ${sqlWhere}) as totalfiltered 
        FROM com_main LEFT JOIN
        list_state ON com_main.State =  list_state.ListID INNER JOIN
        list_country ON com_main.Country = list_country.ListID
        WHERE (com_main.ComID > 0)`
    ...
    if (search == "") {
        ...
        sqlWhere = sqlWhere + ` AND com_main.CompanyName LIKE '%${search}%'`;
